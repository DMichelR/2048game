...
drawNonEmptyCell :: Int -> (Float, Float) -> (Float, Float) -> Float -> Picture
drawNonEmptyCell value (x, y) (w, h) cellSize =
  Pictures [drawRect (getColor value) (x, y) (w, h), drawValue (fromIntegral value) (x - (cellSize / 8), y) cellSize]
...
drawCell value x y size cellSize =
  if value == 0
    ...
    else drawNonEmptyCell value (cellPos x 1.5, cellPos y 1.5) (floatToInt boardSize, floatToInt boardSize) cellSize
  where
